...
struct CommonExampleOptions
{
	...
	CommonExampleOptions(struct GUIHelperInterface* helper, int option = 0)
		: m_guiHelper(helper),
		  ...
		  m_fileName(nullptr),
		  m_sharedMem(nullptr),
		  m_commandProcessorCreation(nullptr),
...
{
public:
	using CreateFunc = class PhysicsInterface*(CommonExampleOptions& options);
	...
	virtual void physicsDebugDraw(int debugFlags) = 0;
	//for now we reuse the flags in Bullet/src/LinearMath/btIDebugDraw.h
...
PhysicsInterface* StandaloneExampleCreateFunc(CommonExampleOptions& options);
