...
{
public:
	GUICombo(const char* label, std::vector<const char*> items, std::function<void()> onChange)
		: mLabel(label), mItems(std::move(items)), mOnChangeCallback(std::move(onChange))
	{
	}

	void Render() override
	{
		if (ImGui::Combo(mLabel, &mCurrentItem, mItems.data(), static_cast<int>(mItems.size())))
		{
			mOnChangeCallback();
		}
	}
