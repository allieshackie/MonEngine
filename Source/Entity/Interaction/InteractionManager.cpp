...
glm::vec3 InteractionManager::_CalculateMouseRay(LLGL::Offset2D mousePos) const
{
	...
	const auto ray = normalize(rayWorld);
...
glm::vec3 InteractionManager::_GetPlaneNormal(const RenderObject* obj) const
{
	...
	return normalize(cross(widthVector, heightVector));
}

bool InteractionManager::_IntersectTest(RenderObject& obj, glm::vec3 mouseRay)
{
	const auto& vertices = obj.GetVertices();
	const auto model = obj.GetModel();
	...
	// plane values
	const glm::vec3 position = model * glm::vec4(vertices[0].position, 1.0);
	//const auto normal = _GetPlaneNormal(&obj);
	...
	const auto normal = normalize(cross(widthVector, heightVector));

	// ray-plane intersection check
	const auto numerator = dot(position - mCamera.GetPosition(), normal);
	const auto denominator = dot(mouseRay, normal);
	const auto t = numerator / denominator;
	// ray/plane intersect
	if (t >= 0)
	{
		// check if within bounds
		const auto intersectionPoint = mCamera.GetPosition() + t * mouseRay;
		const auto v = intersectionPoint - position;

		const auto width = length(widthVector);
		const auto height = length(heightVector);

		const auto proj1 = dot(v, widthVector) / width;
		const auto proj2 = dot(v, heightVector) / height;
