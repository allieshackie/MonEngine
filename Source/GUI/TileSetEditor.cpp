...
{
    ResourceManager::GetInstance()->CreateTile(textureName, { 400.0f, 160.0f }, { 100.0f, 100.0f });
    mCurrentSprite = ResourceManager::GetInstance()->GetLatestTile();
...
void TileSetEditor::_LoadMapMenu(bool* p_open)
{
    ...
    if (ImGui::Begin("Load Map", p_open, ImGuiWindowFlags_MenuBar))
    {
        ...
        if (openMap)
        {
            mCurrentMap = new Map({ 200, 200 }, true);
            mCurrentMap->LoadMap(mapFileNames[current_map_selected]);
            _CreateMapDebugGrid(*mCurrentMap);
            openMap = false;
            show_load_map_menu = false;
            auto mapName = mCurrentMap->GetTileSetDescription().getTexturePath();

            if (ResourceManager::GetInstance()->CreateSimpleOpenGLTexture(mapName, &mPalletteTextureId, &mPalletteTextureWidth, &mPalletteTextureHeight))
            {
...
void TileSetEditor::_PalletteMenu(bool* p_open)
{
    ...
    if (ImGui::Begin("Pallette", p_open, mWindowFlags))
    {
        ...
        ImGui::SameLine();
        if (ImGui::ArrowButton("##up", ImGuiDir_Up)) { current_brush_index++; }
        ImGui::SameLine();
        if (ImGui::ArrowButton("##down", ImGuiDir_Down)) { current_brush_index--; }

        ImGui::Spacing();
        ImGui::Text("Current Brush: ");
        auto texClip = mCurrentMap->GetTileSetDescription().GetClipForTile(current_brush_index);
        ImGui::Image((void*)(intptr_t)mPalletteTextureId, ImVec2(mPalletteTextureWidth, mPalletteTextureHeight), ImVec2(texClip.z, texClip.w), ImVec2(texClip.x, texClip.y));
...
void TileSetEditor::_CreateMapDebugGrid(const Map& map)
{
    ...
    ResourceManager::GetInstance()->CreateGrid(sideA, sideB, rows, columns, { 255,255, 255 });
}
