...
void ImGui_ImplDX11_RenderDrawData(ImDrawData* draw_data)
{
	...
	// Our visible imgui space lies from draw_data->DisplayPos (top left) to draw_data->DisplayPos+data_data->DisplaySize (bottom right). DisplayPos is (0,0) for single viewport apps.
	{
		...
		if (ctx->Map(bd->pVertexConstantBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped_resource) != S_OK)
			return;
		auto constant_buffer = static_cast<VERTEX_CONSTANT_BUFFER*>(mapped_resource.pData);
	...
	for (int n = 0; n < draw_data->CmdListsCount; n++)
	{
		...
		for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
		{
			...
			else
			{
				...
				const D3D11_RECT r = {
					static_cast<LONG>(pcmd->ClipRect.x - clip_off.x), static_cast<LONG>(pcmd->ClipRect.y - clip_off.y),
					static_cast<LONG>(pcmd->ClipRect.z - clip_off.x), static_cast<LONG>(pcmd->ClipRect.w - clip_off.y)
				};
